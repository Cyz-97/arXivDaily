...
% \yan{Here, why do we need to model sequence-to-sequence prediction is not explained. Maybe it should be emphasized.}
...
Inspired by the u-shaped architectural design, the self-attentive feature encoded by Transformers is then upsampled to be combined with different high-resolution CNN features skipped from the encoding path, for enabling precise localization. %which contain detailed localization information.
We show that such a design allows our framework to preserve the advantages of Transformers and %also sit well with 
also benefit medical image segmentation. % by leveraging low-level features with detailed localization information.
...
Following~\cite{dosovitskiy2020image}, we first perform tokenization by reshaping the input $\bm{\mathrm{x}}$ into a sequence of flattened 2D patches \{$\bm{\mathrm{x}}^i_p \in \mathbb{R}^{P^2 \cdot C}|i=1,..,N\}$, where each patch is of size $P \times P$ and $N=\frac{HW}{P^2}$ is the number of image patches (\emph{i.e.}, the input sequence length). % $H \times W$ 
...
% every block, and residual connections after every block, unless otherwise specified.
\begin{align}
...
% Compared to \emph{1-step upsampler}, CUP  enables feature aggregation at different resolution levels which can be greatly beneficial for medical image segmentation tasks. 
...
% \noindent\textbf{Synapse multi-organ segmentation dataset\footnote{https://www.synapse.org/#!Synapse:syn3193805/wiki/217789}.}  
...
% We compare the proposed TransUNet design, \emph{i.e.}, three skip-connections (illustrated in Figure \ref{fig:framework}) with zero skip and one skip and report results in Figure \ref{fig:abl:skip}. 
...
% recovers low-level details better by leveraging skip-connection, 
